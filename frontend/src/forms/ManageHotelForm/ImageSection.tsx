...
export default function ImageSection() {
    ...
    const handleDelete = (e: React.FormEvent,imageUrl:string) => {
        e.preventDefault()
                    ...
                    <div className="grid grid-cols-6 gap-4">
                        {existingImageUrls.map((url,index) => (
                            <div key={index} className="relative group">
                                ...
                                <button onSubmit={(e)=>handleDelete(e,url)} className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 text-white">Delete</button>
                            </div>
                        ))}
                    </div>
                )}
